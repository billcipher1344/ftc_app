...
public class PIDController {
     ...
     * @see #setDelay
     */
    public PIDController(int setpoint, int msdelay, Object lock) {
        this.setpoint = setpoint;
        this.msdelay = msdelay;
        this.lock = lock;
    }

    public void setTelemetry(String tag, Telemetry telemetry) {
        this.tag = tag;
    ...
    public int doPID(int processVariable){
        ...
        if(canDebug()) {
            debug.addLine(tag + " PID")
                    .addData("e", error)
                    .addData("p", Kp*error)
                    .addData("i", integral)
                    .addData("d", Kd*derivative);
        }
